...
void CBaseHudChat::MsgFunc_SayText2( bf_read &msg )
{
	...
	if (!Q_wcscmp(szBuf[4], L"HOME"))
		g_pVGuiLocalize->ConvertANSIToUnicode(GetGlobalTeam(TEAM_HOME)->GetCode(), szBuf[4], sizeof(szBuf[4]));
	else if (!Q_wcscmp(szBuf[4], L"AWAY"))
		g_pVGuiLocalize->ConvertANSIToUnicode(GetGlobalTeam(TEAM_AWAY)->GetCode(), szBuf[4], sizeof(szBuf[4]));
