...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMainStatus::CMainStatus(IViewPort *pViewPort) : Frame( NULL, PANEL_MAINSTATUS )
{
	...
	m_pStatusBarLabel = new Label( m_pTopBar, "topbar", "" );
	...
	LoadControlSettings("Resource/UI/MainStatus.res");
...
//-----------------------------------------------------------------------------
void CMainStatus::ApplySchemeSettings(IScheme *pScheme)
{
	BaseClass::ApplySchemeSettings( pScheme );
	SetBgColor(Color( 0,0,0,0 ) ); // make the background transparent
	m_pTopBar->SetBgColor(Color( 0,0,0,0 ));		//thick bar that "THROW IN" gets written onto
	SetPaintBorderEnabled(false);
	SetBorder( NULL );
	HFont hFont = pScheme->GetFont( "IOSStatusFont" );
	m_pStatusBarLabel->SetFont(hFont);
...
//-----------------------------------------------------------------------------
void CMainStatus::PerformLayout()
{
	int w,h;
	GetHudSize(w, h);
...
//-----------------------------------------------------------------------------
void CMainStatus::SetLogoImage(const char *image)
{
	if ( m_pBannerImage )
	{
...
//-----------------------------------------------------------------------------
void CMainStatus::SetLabelText(const char *textEntryName, const char *text)
{
...
// Purpose: Sets the text of a control by name
//-----------------------------------------------------------------------------
void CMainStatus::SetLabelText(const char *textEntryName, wchar_t *text)
{
	Label *entry = dynamic_cast<Label *>(FindChildByName(textEntryName));
	if (entry)
	{
...
// Purpose: Sets the text of a control by name
//-----------------------------------------------------------------------------
void CMainStatus::MoveLabelToFront(const char *textEntryName)
{
	Label *entry = dynamic_cast<Label *>(FindChildByName(textEntryName));
	if (entry)
	{
...
//-----------------------------------------------------------------------------
void CMainStatus::ShowPanel(bool bShow)
{
	SetVisible( bShow );
	MoveToFront();
}

bool CMainStatus::ShouldShowPlayerLabel( int specmode )
{
	return true;	//ios
}

const static float FADE_TIME = 0.25f;

//-----------------------------------------------------------------------------
// Purpose: Updates the gui, rearranges elements
//-----------------------------------------------------------------------------
void CMainStatus::Update()
{
	int wide, tall;
...
void CMainStatus::SetData(KeyValues *data)
{
	char	text[256];
	...
	m_EndTime = gpGlobals->curtime + 2.0f;
