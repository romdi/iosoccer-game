...
public:
	...
	Vector m_vKeeperSidewaysDiveView;
	Vector m_vKeeperSidewaysDiveHullMin;
	Vector m_vKeeperSidewaysDiveHullMax;
...
#define	HIDEHUD_ALL					( 1<<0 )
#define HIDEHUD_CHAT				( 1<<1 )	// Hide all communication elements (saytext, voice icon, etc)
#define HIDEHUD_SCOREBAR			( 1<<2 )
#define HIDEHUD_POWERSHOTBAR		( 1<<3 )
#define HIDEHUD_PLAYERNAMES			( 1<<4 )
#define	HIDEHUD_WEAPONSELECTION		( 1<<5 )	// Hide ammo count & weapon selection
#define	HIDEHUD_FLASHLIGHT			( 1<<6 )
#define HIDEHUD_HEALTH				( 1<<7 )	// Hide health & armor / suit battery
#define HIDEHUD_PLAYERDEAD			( 1<<8 )	// Hide when local player's dead
#define HIDEHUD_NEEDSUIT			( 1<<9 )	// Hide when the local player doesn't have the HEV suit
#define HIDEHUD_MISCSTATUS			( 1<<10 )	// Hide miscellaneous status elements (trains, pickup history, death notices, etc)
#define	HIDEHUD_CROSSHAIR			( 1<<11 )	// Hide crosshairs
#define	HIDEHUD_VEHICLE_CROSSHAIR	( 1<<12 )	// Hide vehicle crosshair
#define HIDEHUD_INVEHICLE			( 1<<13 )
#define HIDEHUD_BONUS_PROGRESS		( 1<<14 )	// Hide bonus progress display (for bonus map challenges)

#define HIDEHUD_BITCOUNT			15
...
enum PLAYER_ANIM
{
	...
	PLAYER_DIVE_RIGHT,
	PLAYER_SLIDE,
	PLAYER_TACKLED_FORWARD,
	PLAYER_TACKLED_BACKWARD,
	PLAYER_DIVINGHEADER,
};
...
typedef enum
{
	...
	USE_SET = 2, 
	USE_TOGGLE = 3,
	USE_SHOOT = 4,		//IOS! Cant get value passed through to USE function easily anymore.
	USE_PASS = 5,
	USE_GOALKICK = 6,
	USE_POWERSHOT_START = 7,
