...
void CFieldBot::BotRunToBall()
{
	...
	else
	{
		...
		m_cmd.forwardmove = clamp(m_oldcmd.forwardmove + g_IOSRand.RandomFloat(-200, 200) * gpGlobals->frametime * 2, -mp_walkspeed.GetInt(), mp_walkspeed.GetInt());
		m_cmd.sidemove = clamp(m_oldcmd.sidemove + g_IOSRand.RandomFloat(-200, 200) * gpGlobals->frametime * 2, -mp_walkspeed.GetInt(), mp_walkspeed.GetInt());
	}
}
