...
void CPlayerResource::UpdatePlayerData( void )
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		m_NextCardJoin.Set(i, pPl->GetData()->GetNextCardJoin() );
		...
		if (m_nUpdateCounter % 20 == 0)
		{
			...
			m_RedCards.Set(i, max( 0, pPl->GetData()->GetRedCards() ) );
			m_YellowCards.Set(i, max( 0, pPl->GetData()->GetYellowCards() ) );
			m_Fouls.Set(i, max( 0, pPl->GetData()->GetFouls() ) );
			m_FoulsSuffered.Set(i, max( 0, pPl->GetData()->GetFoulsSuffered() ) );
			m_SlidingTackles.Set(i, max( 0, pPl->GetData()->GetSlidingTackles() ) );
			m_SlidingTacklesCompleted.Set(i, max( 0, pPl->GetData()->GetSlidingTacklesCompleted() ) );
			m_GoalsConceded.Set(i, max( 0, pPl->GetData()->GetGoalsConceded() ) );
			m_Shots.Set(i, max( 0, pPl->GetData()->GetShots() ) );
			m_ShotsOnGoal.Set(i, max( 0, pPl->GetData()->GetShotsOnGoal() ) );
			m_PassesCompleted.Set(i, max( 0, pPl->GetData()->GetPassesCompleted() ) );
			m_Interceptions.Set(i, max( 0, pPl->GetData()->GetInterceptions() ) );
			m_Offsides.Set(i, max( 0, pPl->GetData()->GetOffsides() ) );
			m_Goals.Set(i, max( 0, pPl->GetData()->GetGoals() ) );
			m_OwnGoals.Set(i, max( 0, pPl->GetData()->GetOwnGoals() ) );
			m_Assists.Set(i, max( 0, pPl->GetData()->GetAssists() ) );
			m_Possession.Set(i, max( 0, pPl->GetData()->GetPossession() ) );
			m_DistanceCovered.Set(i, max( 0, pPl->GetData()->GetDistanceCovered() ) );
			m_Passes.Set(i, max( 0, pPl->GetData()->GetPasses() ) );
			m_FreeKicks.Set(i, max( 0, pPl->GetData()->GetFreeKicks() ) );
			m_Penalties.Set(i, max( 0, pPl->GetData()->GetPenalties() ) );
			m_Corners.Set(i, max( 0, pPl->GetData()->GetCorners() ) );
			m_ThrowIns.Set(i, max( 0, pPl->GetData()->GetThrowIns() ) );
			m_KeeperSaves.Set(i, max( 0, pPl->GetData()->GetKeeperSaves() ) );
			m_KeeperSavesCaught.Set(i, max( 0, pPl->GetData()->GetKeeperSavesCaught() ) );
			m_GoalKicks.Set(i, max( 0, pPl->GetData()->GetGoalKicks() ) );
