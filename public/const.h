...
#define FL_NO_X_MOVEMENT		(1<<3) // player can't move in direction of x-axis
#define FL_NO_Y_MOVEMENT		(1<<4)	// player can't move in direction of y-axis
#define FL_FROZEN				(1<<5) // Player is frozen for 3rd person camera
#define FL_ATCONTROLS			(1<<6) // Player can't move, but keeps key inputs for controlling another entity
#define	FL_CLIENT				(1<<7)	// Is a player
#define FL_FAKECLIENT			(1<<8)	// Fake client, simulated server side; don't send network messages to them
#define FL_CARRY				(1<<9)	// IOS keeper carrying the ball.
#define FL_CELEB				(1<<10)	// IOS play celeb anims
#define FL_SHIELD_KEEP_OUT		(1<<11)	// IOS play celeb anims
#define FL_SHIELD_KEEP_IN		(1<<12)	// IOS play celeb anims
#define FL_FREECAM				(1<<13)	// IOS play celeb anims

// NOTE if you move things up, make sure to change this value
#define PLAYER_FLAG_BITS		14

// NON-PLAYER SPECIFIC (i.e., not used by GameMovement or the client .dll ) -- Can still be applied to players, though
#define	FL_INWATER				(1<<15)	// In water
#define	FL_FLY					(1<<16)	// Changes the SV_Movestep() behavior to not need to be on ground
#define	FL_SWIM					(1<<17)	// Changes the SV_Movestep() behavior to not need to be on ground (but stay in water)
#define	FL_CONVEYOR				(1<<18)
#define	FL_NPC					(1<<19)
#define	FL_GODMODE				(1<<20)
#define	FL_NOTARGET				(1<<21)
#define	FL_AIMTARGET			(1<<22)	// set if the crosshair needs to aim onto the entity
#define	FL_PARTIALGROUND		(1<<23)	// not all corners are valid
#define FL_STATICPROP			(1<<24)	// Eetsa static prop!		
#define FL_GRAPHED				(1<<25) // worldgraph has this ent listed as something that blocks a connection
#define FL_GRENADE				(1<<26)
#define FL_STEPMOVEMENT			(1<<27)	// Changes the SV_Movestep() behavior to not do any processing
#define FL_DONTTOUCH			(1<<28)	// Doesn't generate touch functions, generates Untouch() for anything it was touching when this flag was set
#define FL_BASEVELOCITY			(1<<29)	// Base velocity has been applied this frame (used to convert base velocity into momentum)
#define FL_WORLDBRUSH			(1<<30)	// Not moveable/removeable brush entity (really part of the world, but represented as an entity for transparency or something)
#define FL_OBJECT				(1<<31) // Terrible name. This is an object that NPCs should see. Missiles, for example.
#define FL_KILLME				(1<<32)	// This entity is marked for death -- will be freed by game DLL
#define FL_ONFIRE				(1<<33)	// You know...
#define FL_DISSOLVING			(1<<34) // We're dissolving!
#define FL_TRANSRAGDOLL			(1<<35) // In the process of turning into a client side ragdoll.
#define FL_UNBLOCKABLE_BY_PLAYER (1<<36) // pusher that can't be blocked by the player
